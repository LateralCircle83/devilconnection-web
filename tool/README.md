...
|--------|---------|---------|
| `modloader_self_check.mjs` | `npm run check:modloader` | Runs the real `Modloader/mod_loader.js` inside a small browser-like VM harness and checks ASAR path normalization, offset/size validation, and repeated `init()` behavior. |
