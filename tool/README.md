...
|--------|---------|---------|
| `startup_self_check.mjs` | `npm run check:startup` | Runs BrowserShell storage fallback, failed-write retry, initialization/concurrent-write preservation, and save-format compatibility; manager save-key isolation and startup failure; and manager README Markdown rendering in one small VM harness. Use `node tool/startup_self_check.mjs browser-shell`, `node tool/startup_self_check.mjs manager`, or `node tool/startup_self_check.mjs markdown` for a single suite. |
