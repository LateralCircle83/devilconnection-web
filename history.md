...
### 加固：存档管理器导入导出可靠性
- `browser_api.js` 暴露无副作用的共享存档解码器，Tyrano 读取与管理器导入统一识别普通 JSON、URI 编码、旧式 `escape` 及 LZString 压缩格式
- 清除、导出和导入操作会等待 IndexedDB 缓存完成加载，避免大存档尚未进入内存时误报为空或漏操作
- ZIP 导入先校验全部目标存档并拒绝重复键，覆盖已有数据前再次确认；写入 flush 失败时恢复导入前快照，不再让损坏或半导入数据替换现有存档
- 导入错误区分 ZIP 读取、内容校验与浏览器存储写入，导出过程显示进度且生成失败不再静默
- `startup_self_check.mjs` 新增共享解码、storage ready、损坏/重复导入、取消覆盖、写入失败恢复及导出错误提示回归测试

### 修复：窄视口输入后游戏画面上移
- 缩放后的 `[edit]` 输入框获得焦点时，浏览器可能滚动 `overflow:hidden` 的 `#tyrano_base`；输入结束后残留的 `scrollTop` 会让后续画面持续上移并在底部露出黑边
- `electron_latest.js` 在浏览器侧包装 Tyrano `[commit]`，保留原始提交语义，并在提交完成时及下一帧复位游戏容器滚动；三个现有输入场景无需分别修改
- `startup_self_check.mjs` 新增提交返回值/上下文保持及即时、下一帧滚动复位回归测试

## 2026-08-03

### 新增：可选的兼容型 APNG 懒加载模组
- 新增默认关闭的 `dc_safe_apng` 内置模组，玩家可在启动管理器中自行启用；清单支持 `defaultEnabled: false`，其他模组继续保持原有默认勾选行为
- 模组通过资源覆盖确定性替换 APNG 插件及游戏自带登记文件，不依赖运行期轮询抢占标签初始化时序；仓库仅保留玩家实际加载的 ASAR 包
- 启动只串行预热标题 APNG，其余游戏自带动画首次播放时按需串行解码，并在停止播放后释放；相同资源路径共用一次解码结果
- 普通第三方 `[load_apng]` 仍保留原版 eager 等待语义；按需播放不会修改全局 `is_wait`，失败不会永久拉黑且允许后续重试
- APNG 补丁在 Tyrano 尚未就绪时会安全退出，重复加载同版本补丁会复用既有注册表和解码状态，避免外部模组重复加载插件时清空运行期状态
- 小剧场拉幕和连结场景动画保留完整登记，播放标签会等待首次解码后继续；真实浏览器验证了标题动画、小剧场入口和首次拉幕流程
- 新增 `safe_apng_self_check.mjs`，直接解析并执行 ASAR 内文件，覆盖首播时序、第三方 eager、路径去重、失败重试、小剧场拉幕、启动预热预算及包结构完整性
